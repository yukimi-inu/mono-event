...
import { formatNumber, formatResult, formatMemory, generateBenchmarkTable, generateTable } from './utils.js'; // Import generateTable
...
// --- Argument Parsing ---
const args = process.argv.slice(2).map(Number).filter(n => !Number.isNaN(n) && n > 0);
...
const padInit = 9;
const padRegSingle = 18; const padRegMulti = 17;
const padRemFwd = 17; const padRemBwd = 17; const padRemRnd = 17;
const padEmit = 9; const padEmitOnce = 14;
const padMemEmpty = 16; const padMemListeners = 19;
const padComp = 11;
...
const memoryColumns = (key = 'memory') => [{ header: 'Memory (KB/inst)', key, pad: padMemEmpty, formatFn: formatMemory }]; // Default for memory
...
const benchmarkRunners = {
    ...
    2: { name: 'Register (Single Instance)', runner: () => runRegisterSingleBenchmark({ REGISTER_ITERATIONS }), columns: timeColumns() },
    3: { name: 'Register (Multi-Instance)', runner: () => runRegisterMultiBenchmark({ REGISTER_MULTI_INSTANCE_COUNT, LISTENERS_PER_MULTI_INSTANCE }), columns: timeColumns() },
    4: { name: 'Removal (Forward)', runner: () => runRemovalFwdBenchmark({ REMOVAL_ITERATIONS }), columns: timeColumns() },
    5: { name: 'Removal (Backward)', runner: () => runRemovalBwdBenchmark({ REMOVAL_ITERATIONS }), columns: timeColumns() },
    6: { name: 'Removal (Random)', runner: () => runRemovalRndBenchmark({ REMOVAL_ITERATIONS }), columns: timeColumns() },
    7: { name: 'Emission', runner: () => runEmissionBenchmark({ LISTENER_COUNT, ITERATIONS: 50000 }),
         columns: [ // Special columns for emission including restrict
    ...
    8: { name: 'Emission (Once)', runner: () => runEmissionOnceBenchmark({ ONCE_LISTENER_COUNT }), columns: timeColumns() },
    9: { name: 'Memory (Empty)', runner: () => runMemoryEmptyBenchmark({ MEMORY_INSTANCE_COUNT }), columns: memoryColumns() },
   10: { name: 'Memory (With Listeners)', runner: () => runMemoryListenersBenchmark({ MEMORY_LISTENERS, MEMORY_INSTANCE_COUNT_WITH_LISTENERS }), columns: memoryColumns() },
   11: { name: 'Comprehensive Scenario', runner: () => runComprehensiveBenchmark({ COMP_INSTANCE_COUNT, COMP_INITIAL_LISTENERS, COMP_ADD_LISTENERS, COMP_REMOVE_LISTENERS, COMP_FINAL_ADD_LISTENERS, COMP_EMIT_COUNT }), columns: timeColumns() },
};
...
if (!runAll) {
    console.log(`Running only specified benchmarks: ${args.map(id => benchmarkRunners[id]?.name || `Unknown(${id})`).join(', ')}`);
}
...
for (const id in benchmarkRunners) {
    if (shouldRun(Number(id))) {
        ...
        // Runner is now a function that takes no arguments
        ...
        if (scenarioResult) {
             ...
             } else if (id === '7') { // Emission test with Restrict
             ...
             if (intermediateColumns) {
                 if (id === '7') {
                     ...
                     // 通常のライブラリの行を作成
                     if (scenarioResult.mono !== undefined) {
                         rows.push(['mono-event', scenarioResult.mono]);
                     }
                     if (scenarioResult.restrict !== undefined) {
                         rows.push(['Restrict', scenarioResult.restrict]);
                     }
                     if (scenarioResult.ee3 !== undefined) {
                         rows.push(['EventEmitter3', scenarioResult.ee3]);
                     }
                     if (scenarioResult.mitt !== undefined) {
                         rows.push(['mitt', scenarioResult.mitt]);
                     }
                     if (scenarioResult.nano !== undefined) {
                         rows.push(['nanoevents', scenarioResult.nano]);
                     }
                     if (scenarioResult.rxjs !== undefined) {
                         rows.push(['RxJS', scenarioResult.rxjs]);
                     }
                     if (scenarioResult.nodeEvents !== undefined) {
                         rows.push(['Node Events', scenarioResult.nodeEvents]);
                     }
                     if (scenarioResult.eventTarget !== undefined) {
                         rows.push(['EventTarget', scenarioResult.eventTarget]);
                     }
...
const summaryColumns = [
    ...
    { header: `Memory (${MEMORY_LISTENERS} Listeners) (KB/inst)`, key: '10', pad: padMemListeners, formatFn: formatMemory },
...
console.log(generateTable({
    ...
    formatters: [
        (v) => String(v),
        ...summaryColumns.map(c => c.formatFn || ((v) => String(v)))
    ],
...
if (shouldRun(11) && allResults[11]) {
    ...
    if (allResults[11].phaseResults && allResults[11].libDisplayOrder && allResults[11].phaseColumns) {
        ...
        console.log(generateTable({
            ...
            formatters: [
                (v) => String(v),
                ...allResults[11].phaseColumns.map(c => c.formatFn || ((v) => String(v)))
            ],
    ...
    // テーブルデータを準備
    const headers = ['Library', 'Total Time (ms)'];
    const rows = [];
    
    // 各ライブラリの結果を行として追加
    if (allResults[11].mono !== undefined) {
        rows.push(['mono-event', allResults[11].mono]);
    }
    if (allResults[11].ee3 !== undefined) {
        rows.push(['EventEmitter3', allResults[11].ee3]);
    }
    if (allResults[11].mitt !== undefined) {
        rows.push(['mitt', allResults[11].mitt]);
    }
    if (allResults[11].nano !== undefined) {
        rows.push(['nanoevents', allResults[11].nano]);
    }
    if (allResults[11].rxjs !== undefined) {
        rows.push(['RxJS', allResults[11].rxjs]);
    }
    if (allResults[11].nodeEvents !== undefined) {
        rows.push(['Node Events', allResults[11].nodeEvents]);
    }
    if (allResults[11].eventTarget !== undefined) {
        rows.push(['EventTarget', allResults[11].eventTarget]);
    }
