...
export function runComprehensiveBenchmark(config) {
    ...
    function runComprehensiveScenarioWithTiming(libKey, createFn, addFn, removeFn, emitFn, addOnceFn) {
        ...
        const totalTime = measureTimeAverage(() => {
            ...
            start = performance.now();
            const onceCycles = 3; // 「onceをいくつか追加=>実行」を3回繰り返す
            ...
            for (let cycle = 0; cycle < onceCycles; cycle++) {
                ...
                // イベントを発行してonceリスナーをトリガー
                for (let i = 0; i < Math.ceil(COMP_EMIT_COUNT / 3); i++) {
                    for (const item of instances) {
                        emitFn(item.instance, i + COMP_EMIT_COUNT * 3 + cycle * 1000);
                    }
                }
            }
    ...
    results.eventTarget = runComprehensiveScenarioWithTiming(
        ...
        (inst, data) => inst.dispatchEvent(eventObj),
        (inst, h) => inst.addEventListener('event', h, { once: true })
    );

    // --- Special handling for NanoEvents and RxJS ---
    // NanoEvents
    const nanoPhaseTimes = { phase1: 0, phase2: 0, phase3: 0, phase4: 0, phase5: 0, phase6: 0, phase7: 0 };
    results.nano = measureTimeAverage(() => {
        ...
        for (let cycle = 0; cycle < onceCycles; cycle++) {
            // 各サイクルでonceリスナーを追加
            for (let i = 0; i < COMP_INSTANCE_COUNT; i++) {
                for (let j = 0; j < Math.ceil(COMP_INITIAL_LISTENERS / 2); j++) {
                    // nanoeventsはunbind関数を呼び出すことでリスナーを削除するので、
                    // リスナー内で自身を削除する関数を作成してonceを模倣
                    const unbind = instances[i].instance.on('event', (e) => {
                        unbind(); // 自身を削除
                        // 実際の処理はここに書く（ベンチマークでは空関数）
                    });
                }
            }
            ...
            for (let i = 0; i < Math.ceil(COMP_EMIT_COUNT / 3); i++) {
                for (const item of instances) {
                    item.instance.emit('event', i + COMP_EMIT_COUNT * 3 + cycle * 1000);
                }
            }
        }
        end = performance.now();
        nanoPhaseTimes.phase7 += (end - start);
    }, 3);
    phaseResults.nano = {
        phase1: nanoPhaseTimes.phase1 / 3,
        phase2: nanoPhaseTimes.phase2 / 3,
        phase3: nanoPhaseTimes.phase3 / 3,
        phase4: nanoPhaseTimes.phase4 / 3,
        phase5: nanoPhaseTimes.phase5 / 3,
        phase6: nanoPhaseTimes.phase6 / 3,
        phase7: nanoPhaseTimes.phase7 / 3
    }; // Use dot notation

    // RxJS
    const rxjsPhaseTimes = { phase1: 0, phase2: 0, phase3: 0, phase4: 0, phase5: 0, phase6: 0, phase7: 0 };
    results.rxjs = measureTimeAverage(() => {
        ...
        for (let cycle = 0; cycle < onceCycles; cycle++) {
            ...
            for (let i = 0; i < COMP_INSTANCE_COUNT; i++) {
                for (let j = 0; j < Math.ceil(COMP_INITIAL_LISTENERS / 2); j++) {
                    // RxJSはsubscription.unsubscribe()でリスナーを削除するので、
                    // リスナー内で自身を削除する関数を作成してonceを模倣
                    const subscription = instances[i].instance.subscribe(d => {
                        subscription.unsubscribe(); // 自身を削除
                        // 実際の処理はここに書く（ベンチマークでは空関数）
                    });
                }
            }
            ...
            for (let i = 0; i < Math.ceil(COMP_EMIT_COUNT / 3); i++) {
                for (const item of instances) {
                    item.instance.next(i + COMP_EMIT_COUNT * 3 + cycle * 1000);
                }
            }
        }
        end = performance.now();
        rxjsPhaseTimes.phase7 += (end - start);
    }, 3);
    phaseResults.rxjs = {
        phase1: rxjsPhaseTimes.phase1 / 3,
        phase2: rxjsPhaseTimes.phase2 / 3,
        phase3: rxjsPhaseTimes.phase3 / 3,
        phase4: rxjsPhaseTimes.phase4 / 3,
        phase5: rxjsPhaseTimes.phase5 / 3,
        phase6: rxjsPhaseTimes.phase6 / 3,
        phase7: rxjsPhaseTimes.phase7 / 3
    }; // Use dot notation
