// docs/performance/scenarios/removalFwd.js
...
export function runRemovalFwdBenchmark(config) {
    ...
    function runSinglePass() {
        ...
        for (let i = 0; i < REMOVAL_ITERATIONS; i++) {
            const handler = () => {};
            // Keep separate handlers for EventTarget as it requires the exact same function reference for removal
            const eventTargetHandler = () => {};
        ...
        // Measure: Remove listeners forward
        const monoTime = measureTime(() => { for (let i = 0; i < REMOVAL_ITERATIONS; i++) monoRemoveEvent.remove(handlers.mono[i]); });
        const ee3Time = measureTime(() => { for (let i = 0; i < REMOVAL_ITERATIONS; i++) ee3Remove.off('event', handlers.ee3[i]); });
        const mittTime = measureTime(() => { for (let i = 0; i < REMOVAL_ITERATIONS; i++) mittRemove.off('event', handlers.mitt[i]); });
        const nanoTime = measureTime(() => { for (let i = 0; i < REMOVAL_ITERATIONS; i++) handlers.nano[i](); });
        const rxjsTime = measureTime(() => { for (let i = 0; i < REMOVAL_ITERATIONS; i++) handlers.rxjs[i].unsubscribe(); });
        const nodeEventsTime = measureTime(() => { for (let i = 0; i < REMOVAL_ITERATIONS; i++) nodeEmitterRemove.off('event', handlers.nodeEvents[i]); });
        const eventTargetTime = measureTime(() => { for (let i = 0; i < REMOVAL_ITERATIONS; i++) eventTargetRemove.removeEventListener('event', handlers.eventTarget[i]); });
